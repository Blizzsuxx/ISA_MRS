...
export default {
  ...
  mounted() {
    ...
    this.$store.dispatch("APlijekovi/dobaviLijekoveKorisnik",this.id);
    this.$store.dispatch("APApoteke/dobaviApoteku", this.id).then(() => {
      this.apotekaUcitana = true;
      console.log(this.$store.state.APApoteke.apoteka);
    });
    this.$store
        .dispatch("APApoteke/dobaviSlobodneTermine", this.id)
        .then(() => {
